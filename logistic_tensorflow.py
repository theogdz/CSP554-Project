...
if __name__ == '__main__':
    if len(sys.argv) != 1:
        ...
        input_tensor.to_csv('input_tensor.csv', index=False, header = ['vader','textblob','negative','positive'])
    else:
        df = pd.read_csv('input_tensor.csv', header = 0)
    start = time.time()
    trained_model = run(df)
    trained_model.save('tf_logistic.h5')
    runtime = start - time.time()
    print(f"The runtime of tensorflow training is: {-runtime}")
